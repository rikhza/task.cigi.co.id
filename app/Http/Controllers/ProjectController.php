...
class ProjectController extends Controller
{
    ...
    { 
        ...
        if($objUser->workspacePermission() == 'Sub Owner')
        {
      ...
      public function appendTimesheetTaskHTML(Request $request, $slug)
    {
        ...
        if($project)
        {
            ...
            if($task && $selected_dates)
            {
                ...
                {
                    $returnHTML .= '<td><div class="day-time" data-ajax-timesheet-popup="true" data-type="create" data-task-id="' . $task->id . '" data-date="' . $dateobj->format('Y-m-d') . '" data-url="' . route('project.timesheet.create', [
